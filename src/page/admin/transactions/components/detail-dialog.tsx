import { Dialog, DialogFooter, DialogHeader, DialogTitle, DialogOverlay, DialogPortal, DialogClose } from "@/components/ui/dialog";
...
export function TransactionDetailDialog({
  ...
  const userDisplayName = user ? (user.fullName || user.userName || "N/A") : "Đang tải...";
  
  // Get order info
  const order = transaction.orderBookingId ? ordersMap.get(transaction.orderBookingId) : null;
  const orderCode = orderDetail?.code || order?.code || (transaction.orderBookingId ? "Đang tải..." : "N/A");
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-4 bg-muted/30 p-4 rounded-lg border">
              <div>
                <Label className="text-xs font-medium text-muted-foreground">Ngày giao dịch</Label>
                <p className="text-sm font-medium mt-1">{formatDate(transaction.transactionDate)}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <p className={`text-base font-bold mt-1 ${isIncoming ? "text-green-700 dark:text-green-400" : "text-red-700 dark:text-red-400"}`}>
              ...
              <div className="col-span-2">
                ...
                <p className="text-sm mt-1">{transaction.description || "N/A"}</p>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-4 bg-purple-50 p-4 rounded-lg border border-purple-100 dark:bg-purple-950 dark:border-purple-800">
              <div className="col-span-2">
                <Label className="text-xs font-medium text-muted-foreground">Tên khách hàng</Label>
              ...
              {user && (
                <>
                  ...
                  <div>
                    ...
                    <p className="text-sm mt-1">{user.phoneNumber || "N/A"}</p>
                  </div>
                </>
              )}
            </div>
              ...
              {(orderDetail || order) && (
                <>
                  <div>
                    ...
                    <div className="mt-1">
                      <Badge
                        variant={getStatusVariant((orderDetail || order)!.status) as "default" | "secondary" | "destructive" | "outline" | "soft-yellow" | "soft-blue" | "soft-purple" | "soft-indigo" | "soft-green" | "soft-orange" | "soft-red" | "soft-gray"}
                  ...
                  <div>
                    ...
                    <p className="text-sm font-medium mt-1">
                      {vnd(parseFloat((orderDetail || order)?.totalAmount || "0"))} VNĐ
                    </p>
                  </div>
                  <div>
                    ...
                    <div className="mt-1">
                      <Badge
                        variant={getPaymentStatusVariant((orderDetail || order)!.paymentStatus) as "default" | "secondary" | "destructive" | "outline" | "soft-yellow" | "soft-blue" | "soft-purple" | "soft-indigo" | "soft-green" | "soft-orange" | "soft-red" | "soft-gray"}
                        className="whitespace-nowrap"
                      >
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <p className="text-sm mt-1">{formatDate(transaction.createdAt)}</p>
              </div>
              <div>
